...
export default function ShopCategory() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(catProducts.length / PRODUCTS_PER_PAGE),
  );
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 min-w-0">
          ...
          >
            ...
            >
              ...
              <div
                className="h-px bg-gradient-to-r from-border via-border/60 to-transparent"
                aria-hidden
              />
          ...
          <main className="flex-1 min-w-0 w-full lg:w-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center justify-between gap-3 mb-5 sm:mb-6 min-w-0">
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 min-w-0">
                ...
                <span className="font-body text-xs sm:text-sm text-muted-foreground whitespace-nowrap">
      ...
      <div className="p-2.5 sm:p-3.5 flex flex-col flex-1 min-h-0 min-w-0">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 mt-1 sm:mt-1.5 mb-1 sm:mb-2 flex-wrap">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`w-2 h-2 sm:w-2.5 sm:h-2.5 shrink-0 ${i < Math.floor(product.rating) ? "fill-primary text-primary" : "fill-muted text-muted"}`}
            />
          ))}
          <span className="font-body text-[9px] sm:text-[10px] text-muted-foreground ml-0.5">
            ({product.reviews})
          </span>
        </div>
        <div className="flex items-baseline justify-between gap-1 mt-auto pt-1.5 sm:pt-2 min-w-0">
          <p className="font-heading font-bold text-xs sm:text-sm text-primary truncate">
            {formatPrice(product.price)}
          </p>
          {product.oldPrice && (
            <p className="font-body text-[9px] sm:text-[10px] text-muted-foreground line-through shrink-0">
